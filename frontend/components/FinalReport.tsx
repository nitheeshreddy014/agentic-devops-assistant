...
export default function FinalReport({ report }: Props) {
  const causes = (report.probable_causes as any[]) ?? [];
  const fixes = (report.recommended_fixes as any[]) ?? [];
  const citations = (report.runbook_citations as any[]) ?? [];
  ...
  return (
    <div className="bg-gray-900 border border-green-700/40 rounded-xl p-5">
        ...
        <Section title="Probable Causes">
          {causes.map((c: any, i: number) => (
            <div key={i} className="mb-1 flex gap-2 text-sm">
              ...
              <span className="text-gray-200">{c.cause} <span className="text-blue-400 text-xs">({Math.round((c.confidence ?? 0) * 100)}%)</span></span>
            </div>
          ))}
        </Section>
      )}
      ...
      {(report.prevention_recommendations as string[] | undefined)?.length ? (
        <Section title="Prevention">
          {(report.prevention_recommendations as string[]).map((r, i) => (
